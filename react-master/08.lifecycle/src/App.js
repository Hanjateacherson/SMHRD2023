...
1. Mount: 컴포넌트가 화면에 처음 그려진 상태 ex) 서버통신으로 데이터 받아올때 - open api, 자체 server
2. Update: props, state, 부모컴포넌트에 변화가 생겼을 때 리렌더링되는 상태
...
function App() {
  ...
  // 버튼을 클릭했을 때, num1에 +1이 증가되도록 구현해보기
  // - state 값을 변경할 때는 set 함수 활용! 
  ...
  useEffect(()=>{
    ...
    // 감시하고 있는 state 값을 변경하는 함수를 useEffect() 안에 구현하면
