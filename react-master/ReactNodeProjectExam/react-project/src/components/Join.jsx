...
/*
  ...
  3. 'userData' state의 변화를 감지하여 node.js 서버에 전달한다. (useEffect 처리)
    - 이벤트처리: onSubmit
    - 요청url: /user/join
    - 전송방식: post
    - 데이터: userData
  ...
  4. 응답받은 데이터에 따라 페이지를 이동한다.
    ...
    - 실패한 경우 -> '다시 한 번 확인해주세요..' 알림을 띄운 후 회원가입페이지('/join')로 이동한다.
...
const Join = () => {
  ...
  //useRef 초기화 - id, pw, pw2, name, email
  const idRef = useRef()
  const pwRef = useRef()
  const pw2Ref = useRef()
